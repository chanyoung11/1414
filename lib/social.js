...
import { appleBase } from './apple.js';

// 애플은 검사에서만 로컬 가짜 애플(APPLE_AUTH_BASE, localhost 만)을 본다 — lib/apple.js
const JWKS_URL = {
  google: () => 'https://www.googleapis.com/oauth2/v3/certs',
  apple: () => appleBase() + '/auth/keys',
};
const ISSUER = {
...
async function jwks(provider) {
  const url = JWKS_URL[provider]();
  const hit = cache.get(provider);
  if (hit && hit.url === url && Date.now() - hit.at < 3600e3) return hit.keys;
  const r = await fetch(url);
  if (!r.ok) throw new Error('키를 받지 못했어요');
  const { keys } = await r.json();
  cache.set(provider, { keys, at: Date.now(), url });
  return keys;
}
...
export async function verifyIdToken(provider, token, audiences) {
  ...
  return {
    ...
    aud: String(body.aud || ''),   // 어느 클라이언트(웹 Services ID · 앱 번들 id)에서 받은 것인지 — 애플 code 를 바꿀 때 client_id
  };
}
