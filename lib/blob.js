...
export async function presignPut(pathname, contentType, minutes = 10, size = 0) {
  ...
  // 옛 저장소(R2 환경변수가 없을 때만)도 같은 크기 한도를 URL 에 싣는다
  const opts = { pathname, validUntil, access: ACCESS, contentType, addRandomSuffix: false, allowOverwrite: true, ...(size ? { maximumSizeInBytes: size } : {}) };
  try {
...
// 1,000개씩 끊어 남은 것이 없을 때까지(길어도 budgetMs 까지) 돈다. 전에는 하루 3,000개만 봐서, 서명 URL 을 마구 받아
// 줄을 쌓으면 그 뒤에 진짜로 버려진 파일은 며칠씩 밀렸다. 못 지운 것은 하루 뒤로 미루니 같은 날 다시 잡히지 않는다
export async function sweepBlobs(limit = 1000, budgetMs = 60000) {
  const pre = deletablePrefixes();
  const out = { deleted: 0, kept: 0, failed: 0, resized: 0 };
  const t0 = Date.now();
  while (Date.now() - t0 < budgetMs) {
    ...
    const live = await q('select team_id, id, url, pathname, size, type from blobs where url = any($1::text[]) or pathname = any($2::text[])',
    ...
    out.deleted += dead.length - failed.size; out.kept += urls.length - dead.length; out.failed += failed.size;
    out.resized += await refreshLive(live);
    if (rows.length < limit) break;
  }
  return out;
}
// 살아 있는 파일의 크기·타입을 저장소에 있는 그대로 맞춘다. 등록한 id 로 서명 URL 을 다시 받아 다른 파일로 덮고
// 등록을 안 하면 blobs 에는 옛 크기가 남았다 (1KB 로 등록하고 200MB 로 덮기). 서명 URL 은 10분이면 끝나니
// 하루 뒤인 지금 저장소에 있는 것이 마지막 모습이다. 파일이 없으면 그대로 둔다 (/blobs/:id/gone 이 맡는다)
async function refreshLive(live) {
  let n = 0;
  for (let i = 0; i < live.length; i += 8) {
    await Promise.all(live.slice(i, i + 8).map(async (r) => {
      const key = r.pathname || keyOf(r.url);
      if (!key) return;
      const h = await headBlob(key).catch(() => null);
      // 행이 옛 저장소(Vercel) 주소를 가리키면 같은 이름의 R2 파일은 그 행의 파일이 아니다
      if (!h || h.url !== r.url || !(h.size >= 0) || (+r.size === h.size && (!h.contentType || r.type === h.contentType))) return;
      await q('update blobs set size=$3, type=coalesce($4, type) where team_id=$1 and id=$2', [r.team_id, r.id, h.size, h.contentType || null])
        .then(() => { n++; }).catch((e) => console.error('blob resize', r.id, e.message));
    }));
  }
  return n;
}
