...
export async function presignPut(pathname, contentType, minutes = 30) {
  ...
  const tok = await issueSignedToken({ operations: ['put'], pathname, validUntil, access: ACCESS, contentType, addRandomSuffix: false });
  const p = await presignUrl(tok, { operation: 'put', pathname, access: ACCESS, validUntil, contentType, addRandomSuffix: false });
