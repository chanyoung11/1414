...
export async function sendApns(userIds, payload) {
  ...
  const first = await round(HOST, rows.map((r) => r.token));
  // 기본 환경이 거부한 토큰만 반대 환경으로 한 번 더 보낸다.
  ...
  if (first && (first.unknown.length || first.wrongKey.length)) {
    const second = await round(OTHER, [...first.unknown, ...first.wrongKey]);
    const was = new Set(first.unknown);
