...
export async function verifySsv(rawQuery) {
  ...
  // 값은 날것(인코딩된 것)에서 인자마다 따로 푼다. 디코딩한 문자열을 '&' 로 나누면 앱이 정하는 custom_data 에
  // '%26user_id%3D…' 를 넣어 다른 인자를 끼워 넣을 수 있다. 같은 이름이 두 번 오면 믿지 않는다
  const f = new Map();
  for (const kv of rawMsg.split('&')) {
    const i = kv.indexOf('=');
    const k = pctDecode(i < 0 ? kv : kv.slice(0, i)), v = pctDecode(i < 0 ? '' : kv.slice(i + 1));
    if (k == null || v == null || f.has(k)) return { ok: false, why: 'shape' };
    f.set(k, v);
  }
  return {
    ok: true,
