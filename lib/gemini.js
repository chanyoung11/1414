...
export async function transcribeScore(image, opts = {}) {
  ...
  let calls = 1;   // 하루 한도에서 센다 (다시 묻기도 한 번씩 과금된다)
  let parsed;
...
고칠 마디: ${JSON.stringify(ask)}`;
  ...
  return { songs, model, usage, badMeasures: badTotal, finishReason: first.finishReason, calls };
}
...
export async function ocrChordsGemini(images, opts = {}) {
  ...
  for (const im of images) {
    ...
    } catch (e) { out.push({ tokens: [], err: e.message }); }
  }
  return { bands: out, usage, model, title: meta.title, key: meta.key };
}
