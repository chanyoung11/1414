...
[0] 악보에 그려진 마디를 처음부터 끝까지 하나도 빠뜨리지 않는다.
 오선이 여러 줄이면 모든 줄을 읽는다. 중간에 끊지 말 것. 보통 40~60마디가 나온다.
...
export async function transcribeScore(image, opts = {}) {
  ...
  // 박자 검증 → 어긋난 마디만 다시
  const { checkMeasures } = await import('./score.js');
  let badTotal = [];
...
고칠 마디: ${JSON.stringify(ask)}`;
          ...
          for (const f of (fj.fixes || [])) {
            ...
            if (s.measures[idx] && Array.isArray(f.n) && f.n.length) s.measures[idx].n = cleanNotes(f.n);
          }
