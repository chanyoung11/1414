...
// Cloud Run 에서는 키 없이 FCM_PROJECT_ID 와 FCM_USE_METADATA=1 만 둔다. 실행 서비스 계정의 토큰을
// 메타데이터 서버에서 받는다 (Firebase 프로젝트가 서비스 계정 키 발급을 막아 두었다)
...
const METADATA = process.env.FCM_USE_METADATA === '1';
export const fcmConfigured = () => !!(PROJECT && (METADATA || (EMAIL && KEY)));
...
async function accessToken() {
  ...
  if (METADATA) {
    const r = await fetch('http://metadata.google.internal/computeMetadata/v1/instance/service-accounts/default/token?scopes=https://www.googleapis.com/auth/firebase.messaging',
      { headers: { 'Metadata-Flavor': 'Google' } });
    const j = await r.json().catch(() => ({}));
    if (!r.ok || !j.access_token) throw new Error('FCM 인증 실패 (메타데이터)');
  ...
  await Promise.all(rows.map(async (r) => {
    const msg = {
      message: {
        ...
        data: { link: String(payload.link || '#/home'), type: String(payload.type || ''), teamId: String(payload.teamId || '') },
