...
import os, sys, time, json, socket, subprocess, gzip, threading, urllib.request, urllib.error, http.cookiejar
...
def t_scrypt():
  ...
  def probe(n):
    done = threading.Event(); lat = []
    def poll():
      while not done.is_set():
        t = time.time(); urllib.request.urlopen(URL + 'api/health', timeout=30).read(); lat.append(time.time() - t); time.sleep(0.005)
    th = threading.Thread(target=poll); th.start(); time.sleep(0.2)
    def one(i):
      Client().json('POST', 'api/auth/signup', {'username': 'sc%s%02d%s' % (tag, i, n), 'password': 'secret1', 'name': 'x'})
    ws = [threading.Thread(target=one, args=(i,)) for i in range(n)]
    for w in ws: w.start()
    for w in ws: w.join()
    done.set(); th.join()
    return max(lat)
  # 동기 scrypt 일 때 가입 32개로 /api/health 가 200ms 넘게 섰다. 다른 일로 바쁜 기계를 생각해 두 번 중 나은 쪽을 본다
  worst = min(probe(32), probe(32))
  if worst > 0.15: return fail('F146 가입이 몰리는 동안 다른 요청이 %.0fms 섬' % (worst * 1000))
  ...
  print('ok F146 — 가입 32개 동안 다른 요청 최대 %.0fms, 로그인·변경·복구 검사 그대로' % (worst * 1000))
...
def t_presign():
  ...
  print('ok F104 — 서명에 크기(content-length)·10분, 다른 크기 거절, 등록 안 한 파일만 치움 %s' % r.stdout.strip().splitlines()[-1])
...
def t_migrate_library(tmp):
  ...
  items = [{'id': 'a', 'title': ''}, {'id': 'b', 'title': '  '}, {'id': 'c'}, {'id': 'd', 'title': '새 노래', 'key': 'A'},
           {'id': 'e', 'title': '주 이름 찬양', 'key': 'G'}]
  for i in range(3):
  ...
  if uses != 6: return fail('F147 사용 이력이 이상함: %s' % uses)
  doc = sql("select doc from services where team_id=$1 and id='s0'", [t], db=db2)[0]['doc']
  if any(it.get('arrId') for it in doc['items'][:3]) or not all(it.get('arrId') for it in doc['items'][3:]):
    return fail('F147 항목 연결이 이상함: %s' % doc['items'])
  print('ok F147 — 제목 없는 항목 9개에서 곡이 안 생김, 제목 있는 곡 2개만')
...
for t in (t_bad_url_prod_entry, t_gzip, t_scrypt, t_presign, t_delete_chunks): run(t)
try:
  with TempDB() as tmp:
    run(t_plus_plan, tmp)
