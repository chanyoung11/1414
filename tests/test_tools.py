...
def run():
  with sync_playwright() as p:
    ...
    if not L.locator('#sidetool').count(): fail('메트로놈 패널이 안 뜸')
    if '메트로놈' not in L.locator('#sidetool').inner_text(): fail('메트로놈 내용이 아님')
    # 왼쪽에서 나와야 악보를 안 가린다
    box = L.locator('#sidetool').bounding_box()
    if box and box['x'] > 5: fail('왼쪽이 아니라 %spx 에서 나옴' % box['x'])
    ...
    L.click('[data-act="sidetool-close"]'); L.wait_for_timeout(700)

    # 건반
    L.click('[data-act="piano"]'); L.wait_for_timeout(1000)
    if L.locator('#sidetool #pkeys .pk.w').count() != 7: fail('흰 건반이 7개가 아님')
    if L.locator('#sidetool #pkeys .pk.b').count() != 5: fail('검은 건반이 5개가 아님')
    ...
    print('OK — 왼쪽 패널 · 메트로놈(BPM·시작/멈춤·박자) · 건반(7+5키·옥타브)')
    b.close()
